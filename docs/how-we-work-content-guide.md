...
세 단계는 연구실 전체의 개발 방법을 설명합니다. 특정 로봇, 프로젝트, 센서 또는 시연을 단계의 대표로 소개하지 않습니다. 기존 개별 연구 영상 연결은 제거했습니다.

통합 영상이 준비될 때까지 단계별 활동과 상호 연결 설명만 공개합니다. 빈 플레이어, 제작 예정 안내, 내부 검토 상태는 화면에 노출하지 않습니다.
...
현재 모든 영상은 `published: false`이며 식별 정보는 `null`입니다.
