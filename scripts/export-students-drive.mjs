...
export async function exportStudentTemplates(destination) {
  ...
  for (const student of archive) {
    ...
    await mkdir(folder);
    const profile = [
      "published: false", `name: ${student.name}`, "nameKo:", `level: ${student.level}`,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  exportStudentTemplates(destination).then((count) => console.log(`Prepared ${count} unpublished student templates in ${destination}`))
