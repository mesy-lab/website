...
- 실제 Google 인증과 폴더 메타데이터 접근을 검증했고, 사용자 승인 후 자동 업데이트 설정을 등록했다. 최초 홈페이지 배포와 실제 가져오기 결과는 아래 현황에서 구분한다.

Drive에 게시물별 폴더를 만들고, `post.json`과 사진/영상을 함께 넣는다. 사진만으로는 정확한 날짜와 제목, 공개 여부를 알 수 없으므로 작은 메타데이터 파일 하나를 사용한다.

현재 코드는 **Drive API 읽기 전용 가져오기와 선택적으로 활성화하는 예약 배포**를 지원한다. 실제 Google 인증과 폴더 메타데이터 접근은 검증했다. 미디어 다운로드와 게시물 가져오기/배포는 아직 실환경에서 검증하지 않았으며, 현재 로컬 페이지는 기존 10건을 그대로 표시한다. 폴더를 전체 공개할 필요는 없다. 다만 가져온 공개 게시물의 파일은 웹사이트에서 공개된다.
...
- 2026-09-17 사용자 승인 후 `GALLERY_SYNC_MODE=google-drive-api`를 등록했다. 홈페이지 변경 사항과 실제 가져오기/배포 workflow를 반영하고 최초 배포를 검증하는 중이다. 예약 실행은 매시 23분이며 GitHub 실행 지연이 발생할 수 있다.
...
폴더 이름은 영문 소문자, 숫자, 하이픈으로 작성한다. 게시물 ID로 사용되므로 기존 게시물 ID와 중복될 수 없다. 파일 이름은 숫자순으로 정렬되며, 첫 파일이 대표 미디어가 된다. JPEG/PNG/WebP 이미지와 MP4/WebM 영상을 지원한다. 영상은 브라우저 호환성이 좋은 H.264/AAC MP4를 권장한다. 큰 영상은 YouTube를 권장하며 개별 파일은 50 MB 이하여야 한다.
...
가져오기는 Drive 원본을 수정하지 않는다. 날짜/파일/메타데이터에 오류가 있으면 실패하며 기존 목록을 유지한다. 성공하면 `src/data/galleryImported.json`을 새 폴더 내용으로 교체하고 미디어를 `public/media/news/imported/`에 복사한다. 삭제하거나 비공개로 바꾼 글은 다음 가져오기에 목록에서 빠지지만, 이미 복사한 파일은 보수적으로 자동 삭제하지 않으므로 공개 파일 제거가 필요하면 따로 정리하고 재배포해야 한다. 빈 폴더를 가져오면 가져온 목록은 비워지며 기존 2021~2022 아카이브는 유지된다.

클라우드 CI 가져오기 결과는 배포 산출물에만 포함되고 Git 저장소에 자동 커밋하지 않는다. 기존 아카이브 10건은 Drive 내용과 별도로 유지된다. 오래된 공개 미디어의 제거가 필요한 경우 위 파일 정리 주의사항을 따른다. 현재 작업에서는 운영 사이트를 배포하지 않았다.
